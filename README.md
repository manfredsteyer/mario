...
- Find a way to accelerate the falling velocity over time
...
In `src/app/engine/walls.ts`, find out what the first tile is that stops you from falling any further:
...
- `renderX` is the position where the hero is painted in the window
- `scrollOffset` defines how much to scroll

## Task 4: Only Paint View Port

In `src/app/engine/level.ts`, make sure only the viewport is rendered. Invisible columns on the left and right of it should be skipped:
